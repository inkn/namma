...
const onMouseMove = event => {
  if (
    ...
    args.layer !== window.app.canvasStore.eraserArgs.size
  ) {
    if (local.show) {
      local.show.remove();
    }
    ...
    local.show = new paper.Path.Circle({
      center: [0, 0],
      radius: window.app.canvasStore.eraserArgs.size / 2,
      strokeColor: "black",
      fillColor: "white"
    });
    args.lastSize = window.app.canvasStore.eraserArgs.size;
  }
...
const onMouseDown = event => {
  ...
  local.path.strokeWidth = window.app.canvasStore.eraserArgs.size;
